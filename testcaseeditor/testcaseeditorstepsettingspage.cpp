...
class TestcaseEditorStepSettingsWidget::Private
{
  friend class TestcaseEditorStepSettingsWidget;
};

TestcaseEditorStepSettingsWidget::TestcaseEditorStepSettingsWidget(QWidget *parent)
  : QWidget(parent), _d(new Private())
{}

TestcaseEditorStepSettingsWidget::~TestcaseEditorStepSettingsWidget(){
  delete _d;
}

// ------------------ TestcaseEditorStepSettingsPage
TestcaseEditorStepSettingsPage::TestcaseEditorStepSettingsPage(){
...
QWidget* TestcaseEditorStepSettingsPage::createPage(QWidget *parent){
  this->_widget = new TestcaseEditorStepSettingsWidget(parent);
  ...
  if(qvariant_cast<QColor>(settings->value(QLatin1String("StatementColor"))).isValid()==false){
    settings->setValue(QLatin1String("StatementColor"),QColor("darkkhaki"));
  }
  ...
  if(qvariant_cast<QColor>(settings->value(QLatin1String("TimerColor"))).isValid()==false){
    settings->setValue(QLatin1String("TimerColor"),QColor("darkgoldenrod"));
  }
  ...
  if(qvariant_cast<QColor>(settings->value(QLatin1String("TimeoutColor"))).isValid()==false){
    settings->setValue(QLatin1String("TimeoutColor"),QColor("darkgoldenrod"));
  }
  ...
  if(qvariant_cast<QColor>(settings->value(QLatin1String("SleeptimerColor"))).isValid()==false){
    settings->setValue(QLatin1String("SleeptimerColor"),QColor("darkgoldenrod"));
  }
  ...
  if(qvariant_cast<QColor>(settings->value(QLatin1String("MessageColor"))).isValid()==false){
    settings->setValue(QLatin1String("MessageColor"),QColor("darkslateblue"));
  }
  ...
  if(qvariant_cast<QColor>(settings->value(QLatin1String("CheckmessageColor"))).isValid()==false){
    settings->setValue(QLatin1String("CheckmessageColor"),QColor("darkslateblue"));
  }
  ...
  if(qvariant_cast<QColor>(settings->value(QLatin1String("FragmentColor"))).isValid()==false){
    settings->setValue(QLatin1String("FragmentColor"),QColor("khaki"));
  }
  ...
  if(qvariant_cast<QColor>(settings->value(QLatin1String("RegionColor"))).isValid()==false){
    settings->setValue(QLatin1String("RegionColor"),QColor("khaki"));
  }
  ...
  QObject::connect(this->_ui.StatementColor,          &Utils::QtColorButton::colorChanged,
                   this,                              &TestcaseEditorStepSettingsPage::refreshPreview);
...
void TestcaseEditorStepSettingsPage::_initGraph(){
  if (this->_model->sequences().length() == 0){
    previewGraph(this->_model);
  }
}
void TestcaseEditorStepSettingsPage::_refreshPreview(){
...
void TestcaseEditorStepSettingsPage::apply(){
  QSettings * settings = Core::ICore::settings();
  settings->beginGroup(QLatin1String("TestcaseEditor"));
  settings->beginGroup(QLatin1String("Steps"));
...
void TestcaseEditorStepSettingsPage::finish(){
  // Do nothing

  return;
}

bool TestcaseEditorStepSettingsPage::matches(const QString& s) const{
